...
struct MarsView: View {
    var body: some View {
        ZStack {
            ...
            ScrollView {
                planetImage(name: "mars")
                Group {
                    planetTitle(title: "Mars")
                    planetText(text: marsSummary1)
                    planetImage(name: "mars2").padding(.top)
                    planetImageSubtext(text: "This 2016 self-portrait of the Curiosity Mars rover shows the vehicle at the Quela drilling location in the Murray Buttes area on lower Mount Sharp. Courtesy of JPL-Caltech/MSSS/NASA").padding(.bottom)
                    planetText(text: marsSummary2)
                    planetImage(name: "mars3").padding(.top)
                    planetImageSubtext(text: "The European Space Agency's Mars Express mission captured this 2018 image of the Korolev crater, more than 50 miles across and filled with water ice, near the north pole. Courtesy of the ESA/DLR/FU Berlin").padding(.bottom)
